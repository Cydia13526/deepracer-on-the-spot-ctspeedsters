...
class Reward:
    ...
    def reward_function(self, params):
        ...
        def dist_to_racing_line(closest_coords, second_closest_coords, car_coords):
            ...
            if a < 1e-6:  # Threshold for considering points too close
                return min(b, c)  # Use minimum distance to either point
            try:
        ...
        def projected_time(first_index, closest_index, step_count, times_list):
            ...
            if current_expected_time < 1e-6:
                return total_expected_time  # Fallback to total expected time
            return (current_actual_time / current_expected_time) * total_expected_time
        ...
        if direction_diff > 30:
            reward *= max(1e-3, 1 - (direction_diff - 30) / 150)  # Gradual penalty

        # Penalize being too slow (smoothed penalty)
        speed_diff_zero = optimals[2] - speed
        if speed_diff_zero > 0.5:
            reward *= max(1e-3, 1 - (speed_diff_zero - 0.5) / 2)  # Gradual penalty
...
reward_object = Reward()  # verbose=True for debugging
